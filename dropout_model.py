...
class DropoutNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(F.relu(self.conv1(x)))  # Output: 16 channels, 16x16
        x = self.pool(F.relu(self.conv2(x)))  # Output: 32 channels, 8x8
        x = self.pool(F.relu(self.conv3(x)))  # Output: 64 channels, 4x4
